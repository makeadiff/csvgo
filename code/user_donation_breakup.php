...
$donut_data = $sql->getById("SELECT U.id,MU.email,MU.phone,SUM(D.donation_amount) AS amount, C.name AS city, MU.first_name
...
$exdon_data = $sql->getById("SELECT U.id,MU.email,MU.phone,SUM(D.amount) AS amount, C.name AS city, MU.first_name
			FROM $db_donut.users U 
			INNER JOIN $db_donut.cities C ON C.id=U.city_id
			INNER JOIN $db_madapp.User MU ON MU.id=U.madapp_user_id
