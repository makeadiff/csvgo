...
if(isset($_SERVER['HTTP_HOST']) and $_SERVER['HTTP_HOST'] == 'makeadiff.in') {
	...
	$db_donut = 'makeadiff_cfrapp';
	$db_madapp = 'makeadiff_cfrapp';
}

$donut_data = $sql->getById("SELECT U.id,MU.email,MU.phone,COALESCE(SUM(D.donation_amount),0) AS amount, C.name AS city
...
$exdon_data = $sql->getById("SELECT U.id,MU.email,MU.phone,COALESCE(SUM(D.amount),0) AS amount, C.name AS city
			FROM $db_donut.users U 
			INNER JOIN $db_donut.cities C ON C.id=U.city_id
			INNER JOIN $db_madapp.User MU ON MU.id=U.madapp_user_id
