...
$all_applicants = $sql->getById("SELECT U.id, U.name, C.name AS city, G.name AS user_group, U.user_type, V.name AS vertical
                                    FROM User U
        ...
        if($response) {
            foreach($response['data'] as $usr) {
                ...
                if(isset($all_applicants[$user_id])) {
                    ...
                    $madapp_user = $sql->getAssoc("SELECT U.id,U.name, C.name AS city, G.name AS user_group, U.user_type, V.name AS vertical
                                                FROM User U
                                                INNER JOIN City C ON C.id = U.city_id
                                                LEFT JOIN UserGroup UG ON UG.user_id = U.id AND UG.year = 2021
                                                LEFT JOIN `Group` G ON G.id=UG.group_id 
